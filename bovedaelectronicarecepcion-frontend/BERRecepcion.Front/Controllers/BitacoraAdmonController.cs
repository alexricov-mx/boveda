...
{
    [ValidateUser]
    public class BitacoraAdmonController : Controller
    {
        protected readonly IConfiguration _configuration;
        protected readonly IRestUtility _utility;
        private readonly IGenerals _generals;
        public BitacoraAdmonController(IConfiguration configuration, IRestUtility utility, IGenerals generals)
        {
        ...
        [RoleFilter(Roles: "AdministrationLogUsers")]
        ...
        [HttpGet]
        public async Task<IActionResult> BitacoraAdmonTable(DateTime fechaInicial, DateTime fechaFinal, string busqueda, int pageNum = 1)
        {
            try
            {
                ...
                var bitacora = await _utility.GetItem<DataResult<IEnumerable<BitacoraAdmonDto>>>("BitacoraAdmon/GetBitacoraAdmonAsync", param);
                ...
                return PartialView("_BitacoraAdmonTable", bitacora);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> DownloadExcel(DateTime fechaInicial, DateTime fechaFinal, string busqueda)
        {
            try
            {
                ...
                param.Add(new CustomHttpParameter("fechaFinal", fechaFinal.ToString("yyyy-MM-dd")));
                param.Add(new CustomHttpParameter("busqueda", busqueda));
                var bitacora = await _utility.GetItem<DataResult<IEnumerable<BitacoraAdmonDto>>>("BitacoraAdmon/GetAllBitacoraAdmonAsync", param);
                var workbook = new XLWorkbook();
                IXLWorksheet worksheet = workbook.Worksheets.Add("BitacoraAdmon");
                worksheet.Cell(1, 1).Value = "Evento";
                worksheet.Cell(1, 2).Value = "Usuario";
                worksheet.Cell(1, 3).Value = "Descripcion";
                worksheet.Cell(1, 4).Value = "FechaCambio";
                
                var list = bitacora.Data.ToList();
                for (int index = 1; index <= list.Count; index++)
                {
                    worksheet.Cell(index + 1, 1).Value = list[index - 1].Evento;
                    worksheet.Cell(index + 1, 2).Value = list[index - 1].Usuario;
                    worksheet.Cell(index + 1, 3).Value = list[index - 1].Descripcion;                    
                    worksheet.Cell(index + 1, 4).Value = list[index - 1].FechaCambio.ToString("dd/MM/yyyy HH:mm:ss");
                }
                ...
                using (var stream = new MemoryStream())
                {
                    ...
                    return File(content, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "BitácoraAdmon.xlsx");
                }
            }
            catch (Exception)
            {
