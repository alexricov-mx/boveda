$(function () {
    ...
    setOpenMenu("menu_Consulta", "ReportRejectedInvoiceAnalytic")
    ...
    $(document).on("click", "#ExportExcelAll", function (e) {
        ...
        if (dataValid.isValid) {
            ...
            location.href = "Consultas/DownloadExcelGetAllRechazosAnalitico?search=" + search.toString().replaceAll(',', '__') + "&start=" + start + "&end=" + end;
        }
        else {
...
function obtenerRechazosTable(pageNum) {
    ...
    if (dataValid.isValid) {
        $.ajax({
            type: "POST",
            url: "Consultas/obtenerRechazosAnaliticoTable",
            data: {
                start: start,
                end: end,
                search: search,
