...
namespace BERecepcion.Api.Controllers.Instrucciones
{
    ...
    public class PaymentListController : ControllerBase
    {
        ...
        public PaymentListController(IPaymentListRepository paymentListRepository, IConfiguration configuration, IDocumentoFirmadoRepository documentoFirmadoRepository, IESignRepository eSignRepository, IHostEnvironment env, IDocumentosRepository documentosRepository, IBitacoraRepository bitacoraRepository, IUsuariosRepository usuariosRepository, ICorreoRepository correoRepository)
        {
        ...
        public async Task<IActionResult> GetPayments(string Token, int pageSize, string search, int pageNum = 1)
        {
            try
            {
                return Ok(await _paymentListRepository.GetPayments(Token, pageSize, search, pageNum));
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> FirmarAsync([FromBody] DataResult<Externos2Dto> model)
        {
            ...
            {
                Log.Error("PaymentList FirmarAsync: {error}", ex.Message);
                return Problem(null, null, 500, "Error interno", null);
            }
        }
        ...
        public async Task<DataResult<Externos2Dto>> CompletaFirmaAsync([FromBody] DataResult<Externos2Dto> externos)
        {
            ...
            try
            {
                ...
                await _bitacoraRepository.InsertaBitacoraAsync(new BitacoraDto()
                {
                    ...
                    Seccion = "Lista de Pago",
                    Accion = "Firmar",
                    Descripcion = $"Firma de Lista de Pago {externos.Data.paymentList.ListaPago_Id}"
                });
            ...
            {
                Log.Error("PaymentList CompletaFirmaAsync: {error}", ex.Message);
