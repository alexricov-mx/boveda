...
{
    public class OldUsuariosRepository : BaseSQLServerSqlRepository, IOldUsuariosRepository
    {
        private readonly IConfiguration _configuration;
        private readonly IBitacoraAdmonRepository _bitacoraAdmonRepository;
        public OldUsuariosRepository(string cnnString, IConfiguration configuration, IBitacoraAdmonRepository bitacoraAdmonRepository) : base(cnnString)
        {
        ...
        public async Task<DataResult<UsuarioActualizaResponseDto>> ActualizaAdministracionAsync(string UserName, Guid userId)
        {
            // Parametros
            DynamicParameters par = new DynamicParameters();
            par.Add("@usuarioLogeado", UserName);
            ...
            using (IDbConnection db = GetConnection())
            {
                var admin = await db.QueryFirstAsync<string>(sql: "SP_usuario_administra_upemex", param: par, commandType: CommandType.StoredProcedure);
                ...
                if (!string.IsNullOrEmpty(admin))
                {
                    ...
                    responseDto.Mensaje = "Actualización exitosa " + admin;
        ...
        public async Task<DataResult<UsuarioActualizaResponseDto>> ActualizaBloqueaAsync(string UserNameModifier, Guid userId)
        {
            // Parametros
            DynamicParameters par = new DynamicParameters();
            ...
            using (IDbConnection db = GetConnection())
            {
                int blocked = await db.QueryFirstAsync<int>(sql: "SP_usuario_bloquea", param: par, commandType: CommandType.StoredProcedure);
                ...
                if (blocked == 1)
                {
                    responseDto.Mensaje = "Bloqueado";
                }
                else
                {
                    responseDto.Mensaje = "Desbloqueado";
                }
                ...
                using (IDbConnection db = GetConnection())
                {
                    ...
                    using (var tran = db.BeginTransaction())
                    {
                        try
                        {
                            ...
                            parI.Add("@organismID", proveedorInDto.Organismo);
        ...
        public async Task<DataResult<IEnumerable<UsuariosSearchResponseDto>>> GetUsuarioSearchAsync(string Texto)
        {
            // Parametros
            DynamicParameters par = new DynamicParameters();
            par.Add("@Texto", Texto);
            ...
            using (IDbConnection db = GetConnection())
            {
                try
                {
                    ...
                    var getUserList = await db.QueryAsync<UsuariosSearchResponseDto>(sql: "SP_usuario_seleccion_search", param: par, commandType: CommandType.StoredProcedure);

                    if (getUserList != null)
                    {

                        DataResult<IEnumerable<UsuariosSearchResponseDto>> responseDto = new DataResult<IEnumerable<UsuariosSearchResponseDto>>();

                        List<UsuariosSearchResponseDto> responseDataDto = new List<UsuariosSearchResponseDto>();

                        responseDto.Message = "Datos encontrados correctamente";
                        responseDto.Status = System.Net.HttpStatusCode.OK;

                        foreach (var getUser in getUserList)
                        {
                            ...
                            responseDataDto.Add(searchResponseDto);
                        }

                        if (responseDataDto.Count > 0)
                        {
                            foreach (var item in responseDataDto)
                            {
                                string r = "";
                        ...
                        responseDto.Data = responseDataDto;
                    ...
                    else
                    {
                        ...
                        responseDto.Message = "No se encontraron datos del usuario";
                        responseDto.Status = System.Net.HttpStatusCode.BadRequest;
                        responseDto.Data = new List<UsuariosSearchResponseDto>();
                        return responseDto;
                    }
                }
                catch (Exception)
